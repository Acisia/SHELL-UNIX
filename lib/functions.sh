...
# Date   : 31/07/2012
# 
# Description : Liste des fonctions utilisÈs dans les scripts de sauvegarde
#
# Ce fichier est ‡ placer dans ../lib/function.sh
#
###################################################################################################################
#
...
#
###################################################################################################################
# Compte le nombre d'occurence d'un caractËre
# getNbCara $TEXT $CARA
getNbCara(){
	#!/bin/bash
	TEXT="$1"
...
#
###################################################################################################################
#Retourne le login ‡ partir du nom et du prÈnom
# getLoginName $NOM $PRENOM
getLoginName(){
	NOM=$1
	PRENOM=$2
	#Traitement sans accent et sans espace tout en undescore
	NOM=$(echo "$NOM" | tr "‡ÁÈËÍÎÓÔÙˆ˘¸¬«…» ÀŒœ‘÷Ÿ‹ -" "aceeeeiioouuACEEEEIIOOUU__")
	PRENOM=$(echo "$PRENOM" | tr "‡ÁÈËÍÎÓÔÙˆ˘¸¬«…» ÀŒœ‘÷Ÿ‹ -" "aceeeeiioouuACEEEEIIOOUU__")
...
# gÈnÈration de mot de passe allÈatoire
getPasswd() {
	local l=$1
       	[ "$l" == "" ] && l=16
...
# fonction suppression de fichier s'il est prÈsent seulement
